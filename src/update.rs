...
use std::{io::Cursor, fs::{File, self}, env, fmt::{self}, collections::HashMap};
...
impl UpdateBuilder {
    ...
    pub fn new() -> Self {
        Self { verbal: true, github_user: None, github_repo: None, binary_map: HashMap::new() }
    }
    ...
    pub async fn check_for_updates(mut self) -> Result<()> {
        ...
        if online::check(None).is_err() {
            return Err( UpdateError("Update failed: Missing Network connection".to_string()).into() );
        }
        
        let url = format!("https://api.github.com/repos/{}/{}/releases/latest", &self.github_user.as_ref().unwrap(), &self.github_repo.as_ref().unwrap());
        ...
        if os_path == None {
            return Err( UpdateError(format!("No binary was found for this operating system")).into() )
        }
